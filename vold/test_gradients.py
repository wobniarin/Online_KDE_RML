...
# Main script for testing gradient and hessian functions
# 17/2/2021
...
if __name__ == "__main__":
    ...
    ht_est_lst, d_dh_St_lst, d2_dh2_St_lst, ut_lst, ft_yi_estm_lst, d_dh_ft_yi_lst, st_memory_lst = ([] for i in range(7))
    # iteration counter
    it_counter = 0
    for timeperiod in df.index:
        ...
        # EQ-5: gradient of f
        # d_dh_ft_yi_ingrid = d_dh_ft(d_dh_ft_memory, y, yi, lambda_value, current_kde, ht_est_memory, index) # recursive formula - Íngrid
        ...
        # TODO handmade calculation of St to obtain gradient and hessian of St
        St = lambda_value * St_memory - (1-lambda_value) * np.log(ft_y)
        grad_St_numpy = np.gradient(St, dx)
        hessian_St_numpy = np.gradient(grad_St_numpy, dx)
        # EQ 2: gradient of St
        ddh_St = d_dh_St(ut, lambda_value)
        # EQ 6: hessian of St
        ddh2_St = d2_dh2_St(d2_dh2_St_memory, ut, lambda_value)
        # ddh2_St = hessian_St_numpy
